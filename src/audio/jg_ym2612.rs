...
use self::envelope::EnvelopeGenerator;
use self::lfo::LowFrequencyOscillator;
use self::phase::PhaseGenerator;
use self::timer::{TimerA, TimerB, TimerControl, TimerTickEffect};
...
impl FmOperator {
    ...
    fn sample_clock(&mut self, modulation_input: i16) -> i16 {
        let phase = self
            .phase
            .current_phase()
            .wrapping_add_signed(modulation_input);
...
impl Ym2612 {
    ...
    pub fn irq_asserted(&self) -> bool {
        self.timer_a.overflow_flag() || self.timer_b.overflow_flag()
    }
...
impl Default for Ym2612 {
    fn default() -> Self {
        Self::new([false; 6], true, true, Opn2BusyBehavior::Ym3438)
    }
}
