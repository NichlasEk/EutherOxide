...
const DEFAULT_EUTHERTIME_APK_PATH: &str = "/home/nichlas/EutherTime-0.1.2-debug.apk";
const DEFAULT_EUTHERPING_APK_PATH: &str = "/home/nichlas/EutherPing-0.7.0-debug.apk";
...
fn send_euthertime_apk(stream: &mut TcpStream, path: &str) -> io::Result<()> {
    let (apk_path, download_filename) = if path == "/downloads/EutherTime-0.1.0-debug.apk" {
        ...
        let download_filename = if path == "/downloads/EutherTime-0.1.2-debug.apk" {
...
fn send_eutherping_apk(
    ...
    let (apk_path, download_filename) = if path == "/downloads/EutherPing-0.2.0-debug.apk" {
        ...
        let download_filename = if path == "/downloads/EutherPing-0.7.0-debug.apk" {
