...
const DEFAULT_EUTHERPING_APK_PATH: &str = "/home/nichlas/EutherPing-0.4.0-debug.apk";
...
fn send_eutherping_apk(
    ...
    let (apk_path, download_filename) = if path == "/downloads/EutherPing-0.2.0-debug.apk" {
        ...
        let download_filename = if path == "/downloads/EutherPing-0.4.0-debug.apk" {
...
mod tests {
    ...
    fn eutherping_apk_uses_versioned_and_compatibility_download_paths() {
        ...
        assert!(!is_eutherping_apk_download_path(
            "/downloads/EutherPing-0.4.1-debug.apk"
        ));
        assert_eq!(
