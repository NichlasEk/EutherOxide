...
const DEFAULT_EUTHERBOARD_APK_PATH: &str = "/home/nichlas/EutherBoard-0.2.5-debug.apk";
...
fn send_eutherboard_apk(stream: &mut TcpStream, path: &str) -> io::Result<()> {
    ...
    let download_filename = if path == "/downloads/EutherBoard-0.2.5-debug.apk" {
        "EutherBoard-0.2.5-debug.apk"
    } else {
