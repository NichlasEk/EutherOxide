...
const DEFAULT_EUTHERID_APK_PATH: &str = "/home/nichlas/EutherID-0.3.0-release-signed.apk";
...
fn send_eutherid_apk(stream: &mut TcpStream, path: &str) -> io::Result<()> {
    ...
    let download_filename = if path == "/downloads/EutherID-0.3.0-release-signed.apk" {
        "EutherID-0.3.0-release-signed.apk"
    } else {
...
fn is_eutherid_apk_download_path(path: &str) -> bool {
    matches!(
        ...
        "/downloads/eutherid.apk"
            ...
            | "/downloads/EutherID-0.3.0-release-signed.apk"
            | "/downloads/eutherid-release-signed.apk"
    )
}
