...
const DEFAULT_EUTHERSURFER_APK_PATH: &str = "/home/nichlas/EutherSurfer-1.0.5.apk";
...
const DEFAULT_EUTHERPING_APK_PATH: &str = "/home/nichlas/EutherPing-0.8.21-debug.apk";
...
fn send_euthersurfer_apk(stream: &mut TcpStream, path: &str) -> io::Result<()> {
    let (apk_path, download_filename) = match path {
        ...
        _ => (DEFAULT_EUTHERSURFER_APK_PATH, "EutherSurfer-1.0.5.apk"),
    };
    send_android_apk(
        stream,
        Path::new(apk_path),
...
fn send_eutherping_apk(
    ...
    let (apk_path, download_filename) = if path == "/downloads/EutherPing-0.2.0-debug.apk" {
        ...
        let download_filename = if path == "/downloads/EutherPing-0.8.21-debug.apk" {
